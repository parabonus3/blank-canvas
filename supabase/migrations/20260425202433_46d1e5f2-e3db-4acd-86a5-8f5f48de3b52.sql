ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS avatar_flair text NOT NULL DEFAULT 'default',
  ADD COLUMN IF NOT EXISTS avatar_flair_color text;
...
CREATE FUNCTION public.get_room_member_profiles(_room_id uuid)
 RETURNS TABLE(user_id uuid, display_name text, friend_code text, avatar_url text, plan_tier text, avatar_flair text, avatar_flair_color text)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  SELECT p.user_id, p.display_name, p.friend_code, p.avatar_url, p.plan_tier, p.avatar_flair, p.avatar_flair_color
...
CREATE FUNCTION public.get_member_public_stats(_user_id uuid)
 RETURNS TABLE(display_name text, avatar_url text, total_seconds bigint, is_stats_public boolean, plan_tier text, avatar_flair text, avatar_flair_color text)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  ...
  GROUP BY p.display_name, p.avatar_url, p.is_stats_public, p.plan_tier, p.avatar_flair, p.avatar_flair_color
$function$;
